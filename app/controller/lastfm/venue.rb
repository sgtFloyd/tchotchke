  ...
  class Venue
    class << self
      ...
      def get_events( venue, festivals_only = nil )
        Services::LastFM.get( "#{TYPE}.getEvents", !:secure, 'venue'=>venue, 'festivalsonly'=>festivals_only )
      end

      # @see http://www.last.fm/api/show?service=395
      def get_past_events( venue, festivals_only = nil, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getPastEvents", !:secure, 'venue'=>venue, 'festivalsonly'=>festivals_only, 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=396
      def search( venue, country = nil, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.search", !:secure, 'venue'=>venue, 'country'=>country, 'limit'=>limit, 'page'=>page )
      end
