  ...
  class User
    class << self
      ...
      def get_artist_tracks( user, artist, start_timestamp = nil, end_timestamp = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getArtistTracks", !:secure, 'user'=>user, 'artist'=>artist, 'startTimestamp'=>start_timestamp, 'endTimestamp'=>end_timestamp, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=448
      def get_banned_tracks( user, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getBannedTracks", !:secure, 'user'=>user, 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=291
      def get_events( user, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getEvents", !:secure, 'user'=>user, 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=263
      def get_friends( user, recent_tracks = nil, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getFriends", !:secure, 'user'=>user, 'recenttracks'=>recent_tracks 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=344
      def get_info( user = nil )
        Services::LastFM.get( "#{TYPE}.getInfo", !:secure, 'user'=>user )
      end

      # @see http://www.last.fm/api/show?service=329
      def get_loved_tracks( user, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getLovedTracks", !:secure, 'user'=>user, 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=264
      def get_neighbors( user, limit = nil )
        Services::LastFM.get( "#{TYPE}.getNeighbonrs", !:secure, 'user'=>user, 'limit'=>limit )
      end

      # @see http://www.last.fm/api/show?service=444
      def get_new_releases( user, useresc = nil )
        Services::LastFM.get( "#{TYPE}.getNewReleases", !:secure, 'user'=>user, 'useresc'=>useresc )
      end

      # @see http://www.last.fm/api/show?service=343
      def get_past_events( user, limit = nil, page = nil )
        Services::LastFM.get( "#{TYPE}.getPastEvents", !:secure, 'user'=>user, 'limit'=>limit, 'page'=>page )
      end
      ...
      def get_personal_tags( user, tag, tagging_type, limit = nil, page = nil )
        ...
        Services::LastFM.get( "#{TYPE}.getPersonalTags", !:secure, 'user'=>user, 'tag'=>tag, 'limit'=>limit, 'page'=>page )
      end

      # @see http://www.last.fm/api/show?service=313
      def get_playlists( user )
        Services::LastFM.get( "#{TYPE}.getPlaylists", !:secure, 'user'=>user )
      end
